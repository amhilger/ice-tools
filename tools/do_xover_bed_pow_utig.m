...
cvx_begin quiet
    ...
    subject to
        adj_bedpows1 == matches.geo_pow(:,1) + dc_offset(matches.ts(:,1))
        adj_bedpows2 == matches.geo_pow(:,2) + dc_offset(matches.ts(:,2))  
cvx_end
cd(orig_dir)
...
for i = 1:length(transect_names)
    ...
    results.geo_pow_xover = results.rdr_geo_pow + dc_offset(i);
